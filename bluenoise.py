...
def FindLargestVoid(BinaryPattern, StandardDeviation):
    ...
    FilteredArray = np.fft.ifftn(
        ndimage.fourier.fourier_gaussian(np.fft.fftn(np.where(BinaryPattern, 1.0, 0.0)), StandardDeviation)).real;
...
def PlotBinaryPatterns(Texture, nPatternRow, nPatternColumn):
    ...
    for i in range(1, nPattern):
        ...
        pyplot.imshow(np.where(Texture * nPattern < i * np.size(Texture), 1.0, 0.0), cmap="gray",
...
def StoreNoiseTextureLDR(Texture, OutputPNGFilePath, nRank=-1):
    ...
    if (Texture.shape[2] == 1):
        ...
        Texture = Texture[:, :, :4];
    # Ravel width and channel count to meet pypng requirements.txt
...
def GenerateBlueNoiseDatabase(RandomSeedIndexList=range(1), MinResolution=16, MaxResolution=1024,
                              ChannelCountList=[1, 2, 3, 4], StandardDeviation=1.5):
    ...
    while (Resolution <= MaxResolution):
        ...
        for nChannel in ChannelCountList:
            for i in RandomSeedIndexList:
                Texture = np.dstack(
                    [GetVoidAndClusterBlueNoise((Resolution, Resolution), StandardDeviation) for j in range(nChannel)]);
...
def Generate3DBlueNoiseTexture(Width, Height, Depth, nChannel, StandardDeviation=1.5):
    ...
    for i in range(Depth):
        StoreNoiseTextureLDR(Texture[:, :, i, :], path.join(OutputDirectory, "LDR_%s_%d.png" % (LDRFormat, i)),
                             Height * Width * Depth);
        StoreNoiseTextureHDR(Texture[:, :, i, :], path.join(OutputDirectory, "HDR_%s_%d.png" % (HDRFormat, i)),
                             Height * Width * Depth);
...
if (__name__ == "__main__"):
    ...
    Texture = GetVoidAndClusterBlueNoise((10, 10), 1.5, InitialSeedFraction=.5);
    for i in Texture:
        print(i)
